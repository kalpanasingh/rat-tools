...
def checkbit(word, n):
    '''
    This function returns true if the n'th bit is set in the word.
    Input is the word and the bit-number (start counting at 0).
    '''
    mask = 2 ** n
    return (word & mask) > 0
...
def count_bits(dqxx, n):
    '''
    This function returns the number of channels that have bit n set.
    Input is the dqxx  array and the bit (start counting at 0).
    '''
    count = 0
    for lcn in range(0, len(dqxx)):
        if checkbit(dqxx[lcn], n):
            count = count + 1
    return count
...
def is_tube_online(dqxx, lcn):
    '''
    Function that returns a boolean array defining if
    Tube is online or not.
    Input is the array of dqxx words and the lcn number for the tube
    '''
    tube_online_mask = 0x5D09071D  # This is the SNO definition of 'online'
    return (dqxx[lcn] & tube_online_mask) == tube_online_mask


def count_offline_channels(dqxx):
    count = 0
    for lcn in range(0, len(dqxx)):
...
def form_dqxx(dqcr, dqch):
    '''
    Function to form the DQXX bitmaps that show the detector status.
    Input is the DQCR and DQCH words for all channels
    This is pretty ugly, just bit-f****
    '''
    ...
    for lcn in range(0, 9728):
        ...
        dqxx_word = 0
        # 0 DQXX[0] = DQCR[0]
        dqxx_word |= ((not checkbit(dqcr_word, 0)) << 0)
        # 1 DQXX[1] = DQCR[8]
        dqxx_word |= ((not checkbit(dqcr_word, 8)) << 1)
        # 2 DQXX[2] = DQCR[9]
        dqxx_word |= ((not checkbit(dqcr_word, 9)) << 2)
        # 3 DQXX[3] = DQCR[10]
        dqxx_word |= ((not checkbit(dqcr_word, 10)) << 3)
        # 4 DQXX[4] = DQCR[11]
        dqxx_word |= ((not checkbit(dqcr_word, 11)) << 4)
        # 8 DQXX[8] = DQCR[1]
        dqxx_word |= ((not checkbit(dqcr_word, 1)) << 8)
        # 9 DQXX[9] = DQCR[4+db]
        dqxx_word |= ((not checkbit(dqcr_word, 4 + db)) << 9)
        # 10 DQXX[10] = DQCR[3]
        dqxx_word |= ((not checkbit(dqcr_word, 3)) << 10)
        # 16 DQXX[16] = DQCH[2]
        dqxx_word |= ((not checkbit(dqch_word, 2)) << 16)
        # 17 DQXX[17] = DQCH[3]
        dqxx_word |= ((not checkbit(dqch_word, 3)) << 17)
        # 18 DQXX[18] = DQCH[4]
        dqxx_word |= ((not checkbit(dqch_word, 4)) << 18)
        # 19 check for maxed-out threshold ( value of 255 )
        threshold = (dqch_word & 0xff0000) >> 16
        if not (threshold == 255):
            dqxx_word |= (1 << 19)
        # 20 DQXX[20] = DQCH[6]
        dqxx_word |= ((not checkbit(dqch_word, 6)) << 20)
        # 21 DQXX[21] = DQCH[7]
        dqxx_word |= ((not checkbit(dqch_word, 7)) << 21)
        # 22 DQXX[22] = DQCH[8]
        dqxx_word |= ((not checkbit(dqch_word, 8)) << 22)
        # 24 DQXX[24] = DQCR[2]
        dqxx_word |= ((not checkbit(dqcr_word, 2)) << 24)
        # 26 DQXX[26] = DQCR[12+db]
        dqxx_word |= ((not checkbit(dqcr_word, (12 + db))) << 26)
        # 27 DQXX[27] = DQCH[1]
        dqxx_word |= ((not checkbit(dqch_word, 1)) << 27)
        # 28 DQXX[28] = DQCH[0]
        dqxx_word |= ((not checkbit(dqch_word, 0)) << 28)
        # 29 DQXX[29] = DQCH[5]
        dqxx_word |= ((not checkbit(dqch_word, 5)) << 29)
        # 30 DQXX[30] = DQCH[9]
        dqxx_word |= ((not checkbit(dqch_word, 9)) << 30)
        dqxx[lcn] = dqxx_word
    return dqxx
...
def dqxx_print(dqcr, dqch, dqid, runnumber):
    '''
    Function that writes out the SNO-style DQXX file.
    Inputs are the dqcr, dqch and dqid arrays and the runnumber.
    '''
