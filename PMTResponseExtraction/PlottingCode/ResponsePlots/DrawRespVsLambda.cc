////////////////////////////////////////////////////////
/// Draws the response versus wavelength data 
		 ...
		 const double within )
{
  ...
  if( files.empty() == true ) // Load from filesFullData
      ...
      for( uLoop = 0; uLoop < filesFullData.size(); uLoop++ )
	{
	  ...
	  MakeRespVsLambdaGraphs( current, respVsLambda, within );
      ...
      for( uLoop = 0; uLoop < files.size(); uLoop++ )
	{
	  ...
	  TH1D* respVsLambda = new TH1D( files[uLoop], "Wavelength Response", numBins, lowBin, highBin );
