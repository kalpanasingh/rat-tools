////////////////////////////////////////////////////////
/// Draws the reflection versus entry radius
///
/// 15/09/10 - New File
		   ...
		   vector<char*> files )
{
  ...
  if( files.empty() == true ) // Load from filesFullData
      ...
      for( uLoop = 0; uLoop < filesFullData.size(); uLoop++ )
	{
	  ...
	  MakeReflVsEntryPosGraphs( current, respVsEntryPos );
      ...
      for( uLoop = 0; uLoop < files.size(); uLoop++ )
	{
	  ...
	  TH1D* respVsEntryPos = new TH1D( files[uLoop], "Position Reflection", numBins, lowBin, highBin );
  ...
  c1->cd();
  results[0]->GetXaxis()->SetTitle( "Entry Radius [mm]" );
