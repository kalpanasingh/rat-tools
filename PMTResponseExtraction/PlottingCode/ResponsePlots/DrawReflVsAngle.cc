...
{
  vector<char*> files;
  DrawReflVsAngle( files, angleMode, within, normalise );
}
		...
		const bool normalise )
{
  ...
  if( files.empty() == true ) // Load from filesFullData
      ...
      for( uLoop = 0; uLoop < filesFullData.size(); uLoop++ )
	{
	  ...
	  MakeReflVsAngleGraphs( current, respVsAngle, angleMode, within, normalise );
  ...
  results[0]->GetYaxis()->SetTitle( "Reflection [%]" );
