#!/usr/bin/env python
import os, sys, ROOT, rat, string, math
...
def AnalyseData(options):
    ...
    if options.batch:
        ...
        outFile = open("AnalyseData_Position.sh", "w")
        outFile.write(outText)
        outFile.close()

        os.system(batch_params["submit"] + " AnalyseData_Position.sh")
...
def AnalysisFunction(material, fitter1, fitter2):
    ...
    performancePlots_Fitter1 = PerformancePlots_SingleFitter(material, fitter1, ROOT.kRed+1, ROOT.kOrange-3, ROOT.kMagenta+1, fitter1)
    print "\n"
...
def PerformancePlots_SingleFitter(material, fitterName, biasColour, resolutionColour, chiSquaredColour):
    ...
    deltaRPlotsFileName = "AnalyseData_Position_DeltaRPlots_" + material + "_" + fitterName + ".root"
    ...
    for energy in energyList:
        ...
        for yIndex, yPosition in enumerate(yPositionList):
            ...
            performanceList = SingleE_SingleY_SingleZ_SingleFitter(material, energy, yPosition, zPosition, fitterName, deltaRPlotsFileName, resultsTextFileName)
...
def SingleE_SingleY_SingleZ_SingleFitter(material, energy, yPosition, zPosition, fitterName, deltaRPlotsFileName, resultsTextFileName):
    ...
    for ds, run in rat.dsreader(infileName):
        ...
        if not fittedVertex.ContainsPosition():
            continue
        if not fittedVertex.ValidPosition():
            continue
    ...
    if (deltaXPlot.Integral() == 0):
        ...
        resolution = math.sqrt((gaussFitX.GetParameter(2) * gaussFitX.GetParameter(2)) + (gaussFitY.GetParameter(2) * gaussFitY.GetParameter(2)) + (gaussFitZ.GetParameter(2) * gaussFitZ.GetParameter(2)));
        chiSquared = math.sqrt((gaussFitX.GetChisquare() * gaussFitX.GetChisquare()) + (gaussFitY.GetChisquare() * gaussFitY.GetChisquare()) + (gaussFitZ.GetChisquare() * gaussFitZ.GetChisquare()));
		
        del gaussFitX
        del gaussFitY
        del gaussFitZ

    deltaRCanvasName = str( int(energy * 1000) ) + "keV_y=" + str(yPosition) + "mm_z=" + str(zPosition) + "mm"
    deltaRCanvas = ROOT.TCanvas(deltaRCanvasName, deltaRCanvasName, 1500, 900)
    deltaRCanvas.Divide(3, 1)
    deltaRCanvas.cd(1)
    deltaXPlot.Draw()
    deltaRCanvas.cd(2)
    deltaYPlot.Draw()
    ...
    deltaRPlotsFile = ROOT.TFile(deltaRPlotsFileName, "UPDATE")
    deltaRCanvas.Write()
...
def OutputPlots_1Fitter(material, fitter1, performancePlots_Fitter1):
    ...
    pictureDirectory = "./PositionPerformance/images_" + material
    if not os.path.exists(pictureDirectory):
        os.makedirs(pictureDirectory)

    combinedPlotsFileName = "AnalyseData_Position_CombinedPlots_" + material + "_" + fitter1 + ".root"
...
def OutputPlots_2Fitters(material, fitter1, fitter2, performancePlots_Fitter1, performancePlots_Fitter2):
    ...
    combinedPlotsFileName = "AnalyseData_Position_CombinedPlots_" + material + "_" + fitter1 + "+" + fitter2 + ".root"
    combinedPlotsFile = ROOT.TFile(combinedPlotsFileName, "UPDATE")
...
if __name__ == '__main__':
    ...
    parser.add_option("--f1", type = "string", dest = "fitterName1", help = "First Fitter to be Tested (name as specified in Template_Macro_Position.mac), default = likelihoodMethod", default = "likelihoodMethod")
    parser.add_option("--f2", type = "string", dest = "fitterName2", help = "(Optional) Second Fitter to be Tested (name as specified in Template_Macro_Position.mac), default = nearAVMethod", default = "nearAVMethod")
