...
initial_offset_to_x = 18.0 + 0.0#1.55
separation = 36
...
for rotation in rotational_positions:
    ...
    x = math.sin(math.radians(rotation)) * rope_radial_pos
    y = math.cos(math.radians(rotation)) * rope_radial_pos
    roll = rotation
    db["x"].append(float("%.2f" % x))
    db["y"].append(float("%.2f" % y))
    db["z"].append(float("%.2f" % z))
    db["roll"].append(float("%.2f" % (90.0+roll)))
...
for rotation in rotational_positions:
    z = 0.0 
    x = math.sin(math.radians(rotation)) * plate_radial_pos
    y = math.cos(math.radians(rotation)) * plate_radial_pos
    roll = rotation + 180.0
    db["x"].append(float("%.2f" % x))
    db["y"].append(float("%.2f" % y))
    db["z"].append(float("%.2f" % z))
