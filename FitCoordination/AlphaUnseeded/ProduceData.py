#!/usr/bin/env python
import os, sys, string, UnseededUtilities
# Author K Majumdar - 10/05/2012 <Krishanu.Majumdar@physics.ox.ac.uk>
...
def ProduceRunMacFiles(options):
    ...
    for index, energy in enumerate(UnseededUtilities.Energies):
        if index == 0:
            generator = "/generator/vtx/set alpha 0 0 0 " + str(energy)
            outfile = UnseededUtilities.Particles[0]
            hadrons = ""
        elif index == 1:
            generator = "/generator/vtx/set e- 0 0 0 " + str(energy)
            outfile = UnseededUtilities.Particles[1]
